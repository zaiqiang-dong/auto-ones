...
async function autoScrollAndExtract(page, targetDate, keyword, projectName) {
    ...
    while (!foundOlderBug) {
        ...
        for (const bug of currentBugs) {
            ...
            console.log(`  → [DEBUG] 已标记 Bug ${bug.id} 为已处理，当前已处理数量: ${extractedBugIds.size}`);
...
async function extractBugsSmart() {
    ...
    // 第一阶段已经提取了所有详细信息，直接使用结果
    const bugsWithDetails = bugList;
    
    console.log('✓ 所有 Bug 的详细信息已在滚动过程中提取完成\n');
    
    console.log(`\n========== 提取完成 ==========`);
    console.log(`总共处理: ${bugsWithDetails.length} 个Bug`);
